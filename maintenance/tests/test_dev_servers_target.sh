#!/usr/bin/env bash
# dev-servers/lib/common.sh ds_target: a failing tool and a game with no catalog target are
...
#
# Each case builds its own fixture holding a copy of the library and a stub takaro-maint on the
...
fixture() {
  ...
  root="$WORK/$name"
  mkdir -p "$root/dev-servers/lib" "$root/maintenance/bin"
  cp "$REPO_ROOT/dev-servers/lib/common.sh" "$root/dev-servers/lib/common.sh"
  case "$mode" in
    target)
