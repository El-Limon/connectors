...
def render_body(marker: dict[str, str], owned_block: str, existing_body: str | None = None) -> str:
...
def reconcile(
    ...
    *,
    ...
    if existing is None:
        block = render_owned_block(observation, targets)
        if publish:
            created = client.issue_create(
                title_for(observation, targets.name), render_body(marker, block), [identity.LABEL]
            )
            return ReconcileResult("create", int(created["number"]), True)
        return ReconcileResult("create", None, True)
    ...
    block = render_owned_block(observation, targets, state=existing_state(old_body) or "detected")
    new_body = render_body(marker, block, old_body)
    if new_body == old_body:
        return ReconcileResult("noop", number, False, "tracked")
    if publish:
        client.issue_update(number, body=new_body)
    return ReconcileResult("update", number, True)
