...
Every environment value whose name looks like a secret, plus every credential a command resolved
from somewhere else (``--token``, ``gh auth token``), is replaced with ``<redacted>`` in every
stdout/stderr write and in every log line the harness keeps.
"""
...
def remember(*values: str | None) -> None:
    """Register a secret that did not come from the environment. Short values are ignored, as in `secret_values`."""
    for value in values:
...
def forget() -> None:
    """Drop every remembered secret. The test suite calls this between tests; commands never do."""
    _REMEMBERED.clear()


def secret_values(environ: dict[str, str] | None = None) -> list[str]:
    """Values worth hiding, longest first so substrings cannot leak."""
