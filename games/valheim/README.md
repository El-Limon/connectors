...
| Item catalogue | ⚠️ | 821 item prefabs synced. Takaro only asks for this when its sync job runs, so a freshly registered server shows an empty catalogue until then. **Names are Valheim's translation keys** (`item_sword_bronze`), not the words a player reads: Valheim's translator lives in an assembly the server plugin does not compile against. Codes (`SwordBronze`) are exact. `takaro-maint verify --checks items,entities` fails on this until the plugin translates. |
| Entity catalogue | ⚠️ | 101 character prefabs synced, same sync-job timing and the same translation-key names as above. `takaro-maint verify --checks items,entities` fails on this until the plugin translates. |
