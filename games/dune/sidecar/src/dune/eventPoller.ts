...
import { mapPluginEvent, type MappedEvent } from './mapping.js';
...
export interface EventPollerOptions {
  ...
  intervalMs?: number;
  /**
   * How a plugin event becomes a Takaro payload. Injected because the plugin's payloads carry no FLS
   * id — the mapper needs the Postgres join (`pluginJoin.ts`) and the entity catalogue, neither of
   * which belongs in a poller. Defaults to the unjoined `mapPluginEvent`, which is what the
   * plugin-absent tests exercise.
   */
  mapEvent?: (event: { type: string; data: unknown; ts?: unknown }) => MappedEvent | null;
}
...
export class EventPoller {
  ...
  async pollOnce(): Promise<number> {
    ...
    for (const event of events) {
      ...
      let mapped = null;
      try {
        mapped = (this.options.mapEvent ?? mapPluginEvent)(event);
      } catch (err) {
