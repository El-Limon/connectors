#!/usr/bin/env bash
# Builds dist/libtakaro-dune.so inside the debian:bookworm toolchain container.
...
echo "  LD  dist/libtakaro-dune.so"
"$CXX" build/*.o "${LDFLAGS[@]}" -o dist/libtakaro-dune.so
strip --strip-unneeded dist/libtakaro-dune.so 2>/dev/null || true
...
undef="$(nm -D --undefined-only dist/libtakaro-dune.so | awk '$1 == "U" { print $2 }' \
          | grep -vE 'GLIBC|GCC|CXXABI' || true)"
if [ -n "$undef" ]; then
  echo "  !! undefined strong symbols in dist/libtakaro-dune.so:" >&2
  printf '%s\n' "$undef" | sed 's/^/     /' >&2
  echo "     LD_PRELOAD would crash-loop the server; refusing to ship this artefact." >&2
  rm -f dist/libtakaro-dune.so
  exit 1
fi
echo "  OK  no undefined strong symbols outside the C/C++ runtime"
( cd dist && sha256sum libtakaro-dune.so > SHA256SUMS )
echo "built dist/libtakaro-dune.so ($(stat -c %s dist/libtakaro-dune.so) bytes)"
cat dist/SHA256SUMS
