// Shared helpers for the Takaro Dune: Awakening plugin (Linux LD_PRELOAD .so).
// Our own code, ported from the Takaro VEIN plugin tree (which itself came from the Enshrouded one).
#pragma once
...
#else
#define TAKARO_PLUGIN_VERSION "0.1.0" // x-release-please-version
#endif
...
const std::string& ExeDir();       // directory holding DuneSandboxServer-Linux-Shipping
...
// The return type every HTTP handler shares, so events.cpp and query.cpp can both produce one
// without either having to include the other's header.
