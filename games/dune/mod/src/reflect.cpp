...
Reflect::Layout& Reflect::LayMut() { return g_lay; }
...
std::string Reflect::NameToString(const FName& n) {
    // Preferred route on THIS build: decode the FName ourselves out of the located FNamePool. There
    // is no `FName::ToString` symbol here (the binary is stripped), so `g_nameToString` is null on
    // the live process and the engine-call route below is dead code kept only for a hypothetical
    // future build that ships symbols. Decoding is also strictly safer: it cannot re-enter the
    // engine, and a bogus index fails the FNameEntry header check instead of segfaulting inside
    // FName::ToString (which is what a stringify-then-check attempt actually did).
    if (Pool::F().ok) return Pool::NameNumbered(n.Comparison, n.Number);
...
// There are no `X::StaticClass` thunks to call on this build (no function symbols at all), so a
// UClass* is obtained the way this binary allows: identify the class by its exported vtable.
//
// TODO(L2): implement by walking GUObjectArray and matching each object's vtable pointer against the
// `_ZTV…` address, then reading the winner's ClassPrivate. Until GUObjectArray resolves (lane L1's
// string-xref work, see resolve.cpp's GLOBAL("GUObjectArray", ...)) this returns nullptr, which
// degrades exactly the capabilities that need a UClass and is reported as such in /health.
void* Reflect::ClassByVTable(const char* ztvName) {
    uint64_t vt = ztvName ? DynSymAddr(ztvName) : 0;
...
std::string Reflect::Validate() {
    ...
    // here means our layout is wrong, never that Dune renamed something.
    void* actorCls = ClassByVTable("_ZTV6AActor");
    void* objectCls = ClassByVTable("_ZTV7UObject");

    // 1. The UClass found by vtable identity has a NamePrivate holding the expected FName.
    ...
    snprintf(b, sizeof b, "ClassByVTable(_ZTV6AActor)=%p NamePrivate@0x%x matches FName('Actor')", actorCls,
             g_lay.objName);
    add("objectName", ok, b);
    if (!ok) {
    ...
    // without depending on a single Dune symbol.
    ...
    // against FindPropertyByName. Nothing here is assumed: the UE4SS layout ini for other UE games says
...
// offline disassembly of this build (research/2026-09-21-binary-dissection.md); there is no DWARF and no
...
std::string Reflect::LayoutJson() {
    ...
    std::string ref = "{\"source\":\"offline disassembly of DuneSandboxServer-Linux-Shipping (no DWARF, no symtab) "
...
std::string DumpValue(void* obj, void* prop, const std::string& type, const std::string& propName) {
    ...
    if (type == "StrProperty") {
        ...
        FString s;
        memcpy(&s, p, 16);
        // A property dump is not exempt from redaction: the game session's password property holds the join
...
std::string Reflect::DumpStruct(const std::string& name) {
    static const char* kPackages[] = {"/Script/DuneSandbox", "/Script/Engine", "/Script/CoreUObject",
                                      "/Script/FuncomLiveServices"};
    void* s = nullptr;
    std::string pkg;
    ...
    if (!s) {
        ...
        std::vector<void*> all;
        GetObjectsOfClass(ClassByVTable("_ZTV7UObject"), all, true);
        for (void* o : all) {
