...
bool Hooks::SwapVTableSlot(const std::string& name, void* ztvSymbolAddr, size_t slot, void* detour, void** origOut,
    ...
    // GUARD 1 — the typeinfo header must be non-zero. This binary is a PIE: in the ON-DISK image
    // every vtable word is 0 (the target lives in an R_X86_64_RELATIVE addend) and only the loader
    // writes the real pointers. A caller that handed us a file-image address would otherwise
    // "successfully" install a hook over a zero and we would jump to 0 on the game thread.
    if (!MemReadable((const void*)ztvSymbolAddr, 16)) { err = "vtable header is not readable"; return false; }
    if (((const uint64_t*)ztvSymbolAddr)[1] == 0) {
        err = "vtable typeinfo word is 0 - this is the unrelocated file image, not live memory";
        return false;
    }
    void* orig = *slotAddr;
    // GUARD 2 — the slot must currently hold a real function. `.text`-only, not "any executable
    // segment": this binary's single R+X PT_LOAD also spans .dynsym/.rodata/.rela.dyn, so the looser
    // test would accept a typeinfo pointer. When the unwind table parsed we additionally require the
    // address to be a known function entry, which is what catches an off-by-one slot index.
    if (!IsTextAddr((uint64_t)(uintptr_t)orig)) {
        err = "vtable slot does not point into .text";
        return false;
    }
    if (EhFrameAvailable() && !IsFunctionEntry((uint64_t)(uintptr_t)orig)) {
        err = "vtable slot does not point at an .eh_frame function entry (slot index is probably wrong)";
        return false;
    }
    if (orig == detour) { err = "slot already holds our detour"; return false; }
