...
// LANE L2 WATCHDOG. The pump rides `UObject::ProcessEvent`, so it has NO guaranteed cadence: lane
// L1b measured a 17 960 ms gap while the engine was flushing level streaming. Nothing in the plugin
// may depend on cadence for correctness (events carry their own timestamps and are enqueued from the
// hook itself; the /players snapshot reports its own staleness), but a stall must still be *visible*
// rather than inferred, so the gap between consecutive pump entries is measured here and the worst
// one is reported in /health.diagnostics.gameThread.
std::atomic<uint64_t> g_maxStallMs{0};
std::atomic<uint64_t> g_stallsOver1s{0};
std::atomic<uint64_t> g_stallsOver5s{0};
std::atomic<uint64_t> g_lastStallMs{0};
constexpr uint64_t kStallWarnMs = 1000;
...
void OnTick() {
    ...
    {
        uint64_t prev = g_lastTickMs.exchange(now);
        if (prev && now > prev) {
            uint64_t gap = now - prev;
            if (gap >= kStallWarnMs) {
                g_stallsOver1s++;
                g_lastStallMs = gap;
                if (gap >= 5000) g_stallsOver5s++;
                uint64_t worst = g_maxStallMs.load();
                while (gap > worst && !g_maxStallMs.compare_exchange_weak(worst, gap)) {
                }
            }
        }
    }
...
// ⚠️ On Dune neither of these resolves from a symbol table (there is none) — they come from a
// vtable slot index, and the Tick slot index has NOT been established offline. Until it is, the pump
// binds through the LIVE engine object's own vtable instead (see the note in resolve.cpp's want
// table), and these two entries resolve to nothing. That is why the pump reports itself as degraded
...
void GameThread::MarkPumpAvailable(const char* how) {
    if (g_installed) return;
    g_installed = true;
    g_how = how;
    PluginState::Get().SetCapability("gameThread", "degraded", "pump installed, waiting for the first call");
    PluginLog("gamethread: pump available via %s", how);
}

void GameThread::PumpOnce() { OnTick(); }
...
std::string GameThread::StatsJson() {
    ...
    return "{\"installed\":" + std::string(g_installed ? "true" : "false") + ",\"how\":" + JsonStr(g_how) +
           ...
           // The pump has no guaranteed cadence (it rides ProcessEvent). These four numbers are the
           // watchdog: nothing depends on cadence for correctness, but a stall is measured, not
           // guessed. `maxStallMs` of ~18 s on a streaming boot is EXPECTED, not a fault.
           ",\"watchdog\":{\"maxStallMs\":" + std::to_string(g_maxStallMs.load()) +
           ",\"lastStallMs\":" + std::to_string(g_lastStallMs.load()) +
           ",\"stallsOver1s\":" + std::to_string(g_stallsOver1s.load()) +
           ",\"stallsOver5s\":" + std::to_string(g_stallsOver5s.load()) +
           ",\"cadenceIsNotRequired\":\"events are timestamped and enqueued on the hook path; /players "
