...
- **Implemented (lane L1):** `GET /health`, `GET /events`, `GET /debug/gamethread|symbols|object|structs`.
...
|---|---|---|
| `GET /players` | — | `[{gameId,name,steamId,platformId:"steam:<id>",ping,spawned,online:true,connectedAt}]` from `GameState.PlayerArray` |
...
| `GET /players/{id}/inventory` | — | `[{code,name,amount,inventory,slot}]` — VEIN's items are *virtual*: `UBaseInventoryComponent::Items` is an `FInventoryArray` whose own `Items` member is a `TArray<FVirtualItemInstance>`. `amount` is the entry's `Stack`, `name` its `CustomLabel`, and `code` the `UItem` class behind its `TSoftClassPtr`, so it round-trips into `POST /give`. Pure reflection, no call into the game |
