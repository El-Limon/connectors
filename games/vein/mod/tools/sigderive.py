...
Used for the `signature` strategy in src/resolve.cpp, which is the last resort for a build whose
symbol tables do not carry a name we need. Two ways in:

  # the address is already known in this binary (e.g. from another build's symbols)
  python3 tools/sigderive.py --binary VeinServer-Linux-Test --addr 0x4e3cf50

  # a Windows PDB gave an RVA in Vein-Win64-Test.exe: derive there, then *validate* in the Linux
  # .text by hand - the two images are different compilers, so a Windows pattern is only a hint
  python3 tools/sigderive.py --binary Vein-Win64-Test.exe --rva 0x1234567 --pe
