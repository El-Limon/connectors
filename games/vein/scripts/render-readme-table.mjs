...
const ROWS = [
  ...
  { label: 'Player inventory', keys: ['actions.getPlayerInventory'],
    note: 'What the player is carrying. VEIN’s items are virtual stacks, so the amount is the stack size and the name is the item’s own label.',
  ...
  { label: 'Give an item', keys: ['actions.giveItem'],
    note: 'The item appears in the player’s inventory without a relog.',
  ...
  { label: 'Player death event', keys: ['events.player-death'],
    note: 'Reaches Takaro when a player dies. A death with no killer (a fall, for instance) reports the player as their own attacker, because VEIN gives the connector no separate cause.',
    notes: { failed: 'Not working yet — player deaths are not reported to Takaro.' } },
  { label: 'Entity kill event', keys: ['events.entity-killed'],
    note: 'Zombie and animal kills reach Takaro with the creature and the killing player.',
  ...
  { label: 'Log events', keys: ['events.log'],
    ...
    notes: { failed: 'The connector forwards server log lines (redacted), but Takaro does not store log lines as events, so they cannot be searched or used in modules.' } },
  ...
  { label: 'Discord: Discord → game chat', keys: ['discord.discordToGame'],
    ...
    notes: { partial: 'Waiting on one real human post in the linked Discord channel; the path is wired and every other direction works.' } },
  ...
  { label: 'Events while the Takaro connection is down', keys: ['resilience.outageReconnect'],
    note: 'Events that happen while Takaro is unreachable are kept and delivered in order once the connection is back.',
