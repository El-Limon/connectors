...
describe('TakaroWsClient end-to-end with fake Takaro + mock plugin', () => {
  ...
  it('identifies, answers ping, serves requests, forwards events, reconnects with backoff', async () => {
    ...
    expect(frames[0]).toEqual({ type: 'identify', payload: { identityToken: 'vein', registrationToken: 'reg-123' } });
    ...
    expect(resp.payload[0]).toMatchObject({ gameId: MOCK_STEAMID, platformId: `steam:${MOCK_STEAMID}`, name: 'Tester' });
    ...
    expect(ev.payload.data).toEqual({
      player: { gameId: MOCK_STEAMID, name: 'Tester', steamId: MOCK_STEAMID, platformId: `steam:${MOCK_STEAMID}`, ip: '10.0.0.5', ping: 24 },
      msg: 'gg',
      channel: 'global',
