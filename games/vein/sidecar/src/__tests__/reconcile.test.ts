...
import { FileOnlineStore, MemoryOnlineStore } from '../vein/onlineStore.js';
...
describe('online reconciliation', () => {
  ...
  it('tracks forwarded connects in the store and sends player-disconnected on startup for players no longer on the server', async () => {
    ...
    try {
      ...
      expect(b2.eventLocationFallback({ gameId: MOCK_STEAMID })).toEqual({ x: 0, y: 0, z: 0 });
    } finally {
      b2.stopEvents();
      b1.stopEvents();
    }
  ...
  it('keeps players that are still online and disconnect events remove them', async () => {
    ...
    try {
      ...
      b.noteConnectionEvent('player-disconnected', { player: { gameId: MOCK_STEAMID } });
      expect(store.players).toEqual([]);
    } finally {
      b.stopEvents();
    }
