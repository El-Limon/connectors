...
import { FileCursorStore, MemoryCursorStore } from '../vein/cursorStore.js';
import { EventPoller } from '../vein/eventPoller.js';
import { mapPluginEvent } from '../vein/mapping.js';
...
const player = { gameId: MOCK_STEAMID, name: 'Tester', steamId: MOCK_STEAMID };
const takaroPlayer = { gameId: MOCK_STEAMID, name: 'Tester', steamId: MOCK_STEAMID, platformId: `steam:${MOCK_STEAMID}` };
...
describe('event mapping', () => {
  ...
  it('player-death with position and attacker', () => {
    const attacker = { gameId: '76561198000000002', name: 'Bob' };
    expect(mapPluginEvent({ type: 'player-death', data: { player, position: { x: 1, y: 2, z: 3 }, attacker } })).toEqual({
      type: 'player-death',
      data: {
        player: takaroPlayer,
        position: { x: 1, y: 2, z: 3 },
        attacker: { gameId: attacker.gameId, name: 'Bob', steamId: attacker.gameId, platformId: `steam:${attacker.gameId}` },
      },
    });
  });
  ...
  it('log', () => {
    ...
    expect(mapPluginEvent({ type: 'log', data: { line: 'LogVein: saved', level: 'info' } })).toEqual({ type: 'log', data: { msg: 'LogVein: saved' } });
...
describe('EventPoller + cursor persistence', () => {
  ...
  beforeEach(async () => {
    mock = new MockPlugin();
    await mock.start();
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'vein-cursor-'));
  });
  afterEach(async () => {
    await mock.stop();
...
describe('log tail takeover', () => {
  it('bridge enables the tail when the plugin players capability is degraded, and forwards log joins', async () => {
    ...
    await mock.start();
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'vein-log-'));
    const logFile = path.join(dir, 'Vein.log');
    fs.writeFileSync(logFile, '[2026.09.17-15.00.00:000][  0]LogInit: boot\n');
    ...
    try {
      ...
      // Engine noise is never an event; the join only fires on the grammar's join line.
      fs.appendFileSync(logFile, 'LogHttp: Verbose: request complete\n');
      bridge.tailer.poll();
      await bridge.tailer.flush();
      expect(events).toEqual([]);
      fs.appendFileSync(
        logFile,
        `LogNet: Login request: ?Name=Tester?Password=$VEIN_DEV_WORLD_PASSWORD userId: Steam:${MOCK_STEAMID} platform: Steam\nLogNet: Join succeeded: Tester\n`,
      );
      bridge.tailer.poll();
      await bridge.tailer.flush();
      await bridge.poller.pollOnce();
      // Identity comes from the log line, enriched with the plugin's ping/ip.
      expect(events).toEqual([['player-connected', { player: { ...takaroPlayer, ping: 24, ip: '10.0.0.5' } }]]);
