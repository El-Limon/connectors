...
const make = (opts: { senderName?: string; serverName?: string }) =>
  new VeinAdapter(new VeinPluginClient({ baseUrl: mock.url(), token: mock.token }), opts);
...
describe('chat sender name precedence', () => {
  ...
  it('sendMessage passes the resolved sender to the plugin, message text untouched', async () => {
    const adapter = make({ senderName: 'Takaro', serverName: 'Takaro Dev Vein' });
    ...
    await adapter.handleAction('sendMessage', { message: 'psst', opts: { senderNameOverride: 'Shop', recipient: { gameId: MOCK_STEAMID } } });
    expect(mock.lastRequest('POST', '/message')?.body).toEqual({ text: 'psst', recipientGameId: MOCK_STEAMID, senderName: 'Shop' });
  });

  it('falls back to the server name when only TAKARO_SERVER_NAME is set', async () => {
    const adapter = make({ serverName: 'Takaro Dev Vein' });
    await adapter.handleAction('sendMessage', { message: 'hi' });
    expect(mock.lastRequest('POST', '/message')?.body).toEqual({ text: 'hi', senderName: 'Takaro Dev Vein' });
  });
});
