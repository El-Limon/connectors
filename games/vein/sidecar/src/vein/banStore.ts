...
export class FileBanStore implements BanStore {
  ...
  load(): PendingBan[] {
    ...
    try {
      ...
      throw new Error(`Cannot read timed-ban store ${this.file}: ${(err as Error).message}`);
    }
    if (!raw.trim()) return [];
    let parsed: unknown;
    try {
      ...
      throw new Error(`Timed-ban store ${this.file} is corrupt: ${(err as Error).message}`);
    }
    if (!Array.isArray(parsed)) throw new Error(`Timed-ban store ${this.file} is not a JSON array`);
