...
export class PluginHttpError extends Error {
  constructor(
    ...
    super(`Vein plugin HTTP ${status} for ${path}${body ? `: ${truncate(body)}` : ''}`);
  }
}
...
export class PluginUnimplementedError extends PluginHttpError {
  constructor(path: string, body: string) {
    super(501, path, body);
    this.message = `Vein plugin has not implemented ${path} (HTTP 501)${body ? `: ${truncate(body)}` : ''}`;
  }
}
...
/** Typed client for the Vein plugin loopback HTTP API (plugin docs/API.md). */
export class VeinPluginClient {
  ...
  private async request<T>(method: 'GET' | 'POST', path: string, body?: unknown): Promise<T> {
    ...
    try {
      ...
      throw new PluginUnreachableError(`Vein plugin unreachable at ${this.baseUrl}${path}: ${reason}`);
    } finally {
      clearTimeout(timer);
    }
