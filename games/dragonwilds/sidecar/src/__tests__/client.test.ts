...
describe('TakaroWsClient end-to-end with fake Takaro + mock plugin', () => {
  ...
  it('identifies, answers ping, serves requests, forwards events, reconnects with backoff', async () => {
    ...
    const takaro = new TakaroWsClient(
      `ws://127.0.0.1:${port}`,
      { identityToken: 'dragonwilds', registrationToken: 'reg-123' },
    ...
    expect(frames[0]).toEqual({ type: 'identify', payload: { identityToken: 'dragonwilds', registrationToken: 'reg-123' } });
    ...
    expect(resp.payload[0]).toMatchObject({ gameId: MOCK_PUID, platformId: `epic:${MOCK_PUID}`, name: 'Limon' });
    ...
    expect(ev.payload.type).toBe('chat-message');
    expect(ev.payload.data).toEqual({
      player: { gameId: MOCK_PUID, name: 'Limon', epicOnlineServicesId: MOCK_PUID, steamId: '76561198000000001', platformId: `epic:${MOCK_PUID}`, ping: 24 },
      msg: 'gg',
      channel: 'global',
      // The game's own event time travels with the event, so a replay after an outage keeps its original timestamp.
...
/** Finding F10: a gameEvent written to a socket that is not open AND identified is silently lost. */
describe('TakaroWsClient delivery guarantees', () => {
  const cleanups: Array<() => unknown> = [];
  afterEach(async () => {
    for (const c of cleanups.splice(0).reverse()) await c();
  });

  it('refuses to send before identifyResponse and after a dead socket is detected by the ping watchdog', async () => {
    // autoPong:false = a peer that accepts the TCP connection but never answers - the F10 half-dead socket.
    ...
    wss.on('connection', (ws) => {
      ...
      ws.on('message', (raw) => {
        const msg = JSON.parse(raw.toString());
        if (msg.type === 'identify' && identify) ws.send(JSON.stringify({ type: 'identifyResponse', payload: { gameServerId: 'gs-2' } }));
      });
    });
    ...
    const takaro = new TakaroWsClient(
      `ws://127.0.0.1:${port}`,
      { identityToken: 'dragonwilds', registrationToken: 'reg-123' },
      { baseReconnectMs: 50, maxReconnectMs: 100, pingIntervalMs: 20, idleTimeoutMs: 120 },
    );
    cleanups.push(() => takaro.shutdown());
    takaro.connect();

    // Socket open but not identified yet: events must NOT be handed to it.
    await waitFor(() => sockets.length > 0);
    expect(takaro.sendGameEvent('chat-message', { msg: 'too early' })).toBe(false);
    expect(takaro.identified()).toBe(false);

    // Now let identify succeed on the next connection attempt.
    identify = true;
    const gsid = await waitFor(() => takaro.getGameServerId() ?? undefined, 6000);
    expect(gsid).toBe('gs-2');
    expect(takaro.sendGameEvent('chat-message', { msg: 'ok now' })).toBe(true);

    // The peer stops answering pings entirely: the watchdog must terminate and re-identify within seconds,
    // instead of writing frames into the void until a 1006 arrives minutes later.
    const before = sockets.length;
    await waitFor(() => sockets.length > before, 6000);
  });
});
