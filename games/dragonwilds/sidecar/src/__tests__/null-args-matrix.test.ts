...
import { DragonwildsAdapter, expiryOf, playerId } from '../dragonwilds/adapter.js';
...
describe('explicit-null / wrong-type argument matrix', () => {
  it('teleportPlayer tolerates dimension absent, null or wrong-typed', async () => {
    for (const dimension of NULLISH) {
      await adapter.handleAction('teleportPlayer', { gameId: MOCK_PUID, x: 1, y: 2, z: 3, dimension });
      expect(mock.lastRequest('POST', '/teleport')?.body).toEqual({ gameId: MOCK_PUID, x: 1, y: 2, z: 3 });
    }
    // yaw / target nullish are simply omitted
    await adapter.handleAction('teleportPlayer', { gameId: MOCK_PUID, x: 1, y: 2, z: 3, yaw: null, target: null });
    expect(mock.lastRequest('POST', '/teleport')?.body).toEqual({ gameId: MOCK_PUID, x: 1, y: 2, z: 3 });
    // coordinates as numeric strings still work; a missing coordinate is a clean error
    await adapter.handleAction('teleportPlayer', { gameId: MOCK_PUID, x: '1', y: '2', z: '3' });
    expect(mock.lastRequest('POST', '/teleport')?.body).toEqual({ gameId: MOCK_PUID, x: 1, y: 2, z: 3 });
    await expect(adapter.handleAction('teleportPlayer', { gameId: MOCK_PUID, x: null, y: null, z: null })).rejects.toThrow(/numeric/);
  });
  ...
  it('banPlayer tolerates reason and expiresAt absent, null or wrong-typed', async () => {
    for (const reason of NULLISH) {
      for (const expiresAt of NULLISH) {
        await adapter.handleAction('banPlayer', { gameId: MOCK_PUID, reason, expiresAt });
        const body = mock.lastRequest('POST', '/ban')?.body;
        expect(body.gameId).toBe(MOCK_PUID);
    ...
    await adapter.handleAction('banPlayer', { gameId: MOCK_PUID, reason: 'grief', expiresAt: '2030-01-01T00:00:00.000Z' });
    expect(adapter.pendingBans()).toEqual([{ gameId: MOCK_PUID, expiresAt: '2030-01-01T00:00:00.000Z', reason: 'grief' }]);
    // a later permanent ban clears the pending expiry
    await adapter.handleAction('banPlayer', { gameId: MOCK_PUID, reason: null, expiresAt: null });
    expect(adapter.pendingBans()).toEqual([]);
  });

  it('kickPlayer tolerates a null / wrong-typed reason', async () => {
    for (const reason of NULLISH) {
      await adapter.handleAction('kickPlayer', { gameId: MOCK_PUID, reason });
  ...
  it('giveItem tolerates null quality/amount and nested item objects', async () => {
    for (const quality of NULLISH) {
      await adapter.handleAction('giveItem', { gameId: MOCK_PUID, item: 'Item_Log_Oak', amount: 2, quality });
      expect(mock.lastRequest('POST', '/give')?.body).toEqual({ gameId: MOCK_PUID, code: 'Item_Log_Oak', amount: 2 });
    }
    await adapter.handleAction('giveItem', { gameId: MOCK_PUID, item: 'Item_Log_Oak', amount: null });
    expect(mock.lastRequest('POST', '/give')?.body).toEqual({ gameId: MOCK_PUID, code: 'Item_Log_Oak', amount: 1 });
    await adapter.handleAction('giveItem', { gameId: MOCK_PUID, item: { code: 'Item_Log_Oak' }, amount: 3 });
    expect(mock.lastRequest('POST', '/give')?.body).toEqual({ gameId: MOCK_PUID, code: 'Item_Log_Oak', amount: 3 });
  });
  ...
  it('sendMessage tolerates opts and opts.recipient absent, null or wrong-typed', async () => {
    ...
    await adapter.handleAction('sendMessage', { message: 'hi', opts: { recipient: { gameId: MOCK_PUID } } });
    expect(mock.lastRequest('POST', '/message')?.body).toEqual({ text: 'hi', recipientGameId: MOCK_PUID, senderName: 'Server' });
  });

  it('player identifiers: flat, nested player, nested playerRef, platform-prefixed', () => {
    expect(playerId({ gameId: MOCK_PUID })).toBe(MOCK_PUID);
    expect(playerId({ gameId: null, player: { gameId: MOCK_PUID } })).toBe(MOCK_PUID);
    expect(playerId({ player: null, playerRef: { steamId: '76561198000000001' } })).toBe('76561198000000001');
    expect(playerId({ platformId: `epic:${MOCK_PUID}` })).toBe(`epic:${MOCK_PUID}`);
    expect(playerId({ player: { epicOnlineServicesId: MOCK_PUID } })).toBe(MOCK_PUID);
