...
console.log(`Mock Dragonwilds plugin on ${mock.url()} (token ${mock.token})`);
mock.pushEvent('chat-message', { player: mock.players[0], msg: 'mock boot', channel: 'global' });
setInterval(() => mock.pushEvent('log', { msg: `LogDominion: mock heartbeat ${new Date().toISOString()}` }), 30000);
