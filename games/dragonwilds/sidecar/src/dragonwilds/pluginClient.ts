...
export class PluginHttpError extends Error {
  constructor(
    ...
    super(`Dragonwilds plugin HTTP ${status} for ${path}${body ? `: ${truncate(body)}` : ''}`);
  }
}
...
export class PluginUnimplementedError extends PluginHttpError {
  constructor(path: string, body: string) {
    super(501, path, body);
    this.message = `Dragonwilds plugin has not implemented ${path} (HTTP 501)${body ? `: ${truncate(body)}` : ''}`;
  }
}
...
export interface TeleportOptions {
  yaw?: number;
  /** Named teleport target (UTeleportationSubsystem location name) instead of raw coordinates. */
  target?: string;
}

/** Typed client for the Dragonwilds plugin loopback HTTP API (plugin docs/API.md). */
export class DragonwildsPluginClient {
  ...
  sendMessage(text: string, recipientGameId?: string, senderName?: string): Promise<unknown> {
    return this.request('POST', '/message', omitUndefined({ text, recipientGameId, senderName }));
  }
  teleport(gameId: string, x: number, y: number, z: number, opts: TeleportOptions = {}): Promise<unknown> {
    return this.request('POST', '/teleport', omitUndefined({ gameId, x, y, z, yaw: opts.yaw, target: opts.target }));
  }
  give(gameId: string, code: string, amount: number): Promise<unknown> {
    return this.request('POST', '/give', { gameId, code, amount });
  }
  kick(gameId: string, reason?: string): Promise<unknown> {
    return this.request('POST', '/kick', omitUndefined({ gameId, reason }));
  }
  ban(gameId: string, reason?: string): Promise<unknown> {
    return this.request('POST', '/ban', omitUndefined({ gameId, reason }));
  }
  ...
  getItems(search?: string): Promise<unknown[]> {
    return this.request('GET', search ? `/items?search=${encodeURIComponent(search)}` : '/items');
  }
  ...
  private async request<T>(method: 'GET' | 'POST', path: string, body?: unknown): Promise<T> {
    ...
    try {
      ...
      throw new PluginUnreachableError(`Dragonwilds plugin unreachable at ${this.baseUrl}${path}: ${reason}`);
    } finally {
      clearTimeout(timer);
    }
