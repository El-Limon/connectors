...
import { DragonwildsPluginClient, PluginHttpError, PluginUnimplementedError } from './pluginClient.js';
...
/** Strips a `epic:` / `steam:` platform prefix from a Takaro identifier. */
export function stripPlatform(id: string): string {
  return id.replace(/^(epic|steam):/i, '');
}

/**
 * Maps the 17 Takaro generic-connector actions onto the Dragonwilds plugin HTTP API.
 * Returns the Takaro response payload, or throws (ActionError / plugin errors) so the caller answers with an error frame.
 */
export class DragonwildsAdapter {
  ...
  constructor(
    private readonly plugin: DragonwildsPluginClient,
    private readonly options: AdapterOptions = {},
  ) {
    this.pending = options.banStore?.load() ?? [];
  ...
  async handleAction(action: string, args: Args): Promise<unknown> {
    try {
      ...
      if (err instanceof PluginUnimplementedError) {
        throw new ActionError(`Dragonwilds connector cannot perform '${action}': ${err.message}`);
      }
      throw err;
    }
  }
  ...
  private async dispatch(action: string, args: Args): Promise<unknown> {
    switch (action) {
      ...
      case 'giveItem': {
        ...
        // Dragonwilds items have no quality tier: an explicit `quality` (or null) from Takaro is accepted and ignored.
      ...
      case 'sendMessage': {
        ...
        const recipientId =
          str(recipient.gameId) ?? str(recipient.epicOnlineServicesId) ?? str(recipient.steamId) ?? str(recipient.platformId) ?? str(args.recipientGameId);
      ...
      case 'teleportPlayer': {
        ...
        const y = num(args.y);
        const z = num(args.z);
        // `dimension` is always null/absent for Dragonwilds (single world) and is ignored.
  ...
  private async listBans(): Promise<TakaroBan[]> {
    ...
    for (const ban of bans) {
      const pending = byId.get(ban.player.gameId) ?? byId.get(ban.player.epicOnlineServicesId ?? '') ?? byId.get(ban.player.steamId ?? '');
      if (pending) {
  ...
  private async testReachability(): Promise<{ connectable: boolean; reason: string | null }> {
    try {
      ...
      if (status === 'degraded') return { connectable: true, reason: `Dragonwilds plugin degraded${capText ? `; ${capText}` : ''}` };
      return { connectable: false, reason: `Dragonwilds plugin status '${health.status}'${capText ? `; ${capText}` : ''}` };
    } catch (err) {
  ...
  private index(player: TakaroPlayer): boolean {
    ...
    for (const key of [player.gameId, player.epicOnlineServicesId, player.steamId, player.platformId, player.name]) {
  ...
  /** Finds a plugin player by EOS ProductUserId, platformId, SteamID64, character name or platform name. */
  async findPlayer(id: string): Promise<PluginPlayer | undefined> {
    ...
    try {
      players = await this.plugin.getPlayers();
    } catch {
      return undefined;
    }
    ...
    return (
      ...
      players.find((p) => p.epicOnlineServicesId?.toLowerCase() === needle) ??
  ...
  /** Plugin endpoints take the plugin's gameId (PUID); offline players (ban/unban) fall back to the id as given. */
...
export function playerId(args: Args): string {
  for (const source of [args, asRecord(args.player), asRecord(args.playerRef)]) {
    const id = str(source.gameId) ?? str(source.epicOnlineServicesId) ?? str(source.steamId) ?? str(source.platformId);
    if (id) return id;
  }
