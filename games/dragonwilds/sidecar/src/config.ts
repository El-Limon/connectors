...
export const DEFAULT_LOG_FILE = '/home/ubuntu/Steam/RSDragonwilds/Saved/Logs/RSDragonwilds.log';
...
export function loadConfig(env: Env = process.env): SidecarConfig {
  ...
  const mode = (env.DRAGONWILDS_LOG_TAIL || 'auto').toLowerCase();
  if (mode !== 'auto' && mode !== 'always' && mode !== 'never') {
    throw new Error(`DRAGONWILDS_LOG_TAIL must be auto|always|never, got '${mode}'`);
  }

  const logEvents = (env.DRAGONWILDS_LOG_EVENTS || 'filtered').toLowerCase();
  if (logEvents !== 'all' && logEvents !== 'filtered' && logEvents !== 'none') {
    throw new Error(`DRAGONWILDS_LOG_EVENTS must be all|filtered|none, got '${logEvents}'`);
  }
  ...
  return {
    ...
    identityToken: env.TAKARO_IDENTITY_TOKEN || 'dragonwilds',
    registrationToken,
    serverName: env.TAKARO_SERVER_NAME || 'Takaro Dev Dragonwilds',
    ...
    logFile: env.DRAGONWILDS_LOG_FILE || DEFAULT_LOG_FILE,
    logTailMode: mode,
    logEvents,
