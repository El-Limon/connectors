...
void* Reflect::StaticClass(const char* symName) {
    auto fn = (FnStaticClass)(uintptr_t)Sym::Addr(symName);
...
std::string Reflect::Validate() {
    ...
    void* chatCls = StaticClass("UPlayerChatComponent::StaticClass");
    FName want = MakeName("PlayerChatComponent");
    ...
    bool ok = chatCls && want.Comparison && nameAt(chatCls, g_lay.objName) == want;
    if (!ok && chatCls && want.Comparison) {
        for (uint32_t off = 0x10; off <= 0x30; off += 4) {
            if (nameAt(chatCls, off) == want) {
                g_lay.objName = off;
                ok = true;
                break;
            }
    ...
    snprintf(b, sizeof b, "UPlayerChatComponent::StaticClass()=%p NamePrivate@0x%x matches FName('PlayerChatComponent')",
             chatCls, g_lay.objName);
    add("objectName", ok, b);
    if (!ok) {
    ...
    // 2. CDO: ClassDefaultObject offset, cross-checked through the CDO's own Class pointer.
    FName wantCdo = MakeName("Default__PlayerChatComponent");
    void* cdo = ClassDefaultObject(chatCls);
    ok = cdo && ObjClass(cdo) == chatCls && (!wantCdo.Comparison || nameAt(cdo, g_lay.objName) == wantCdo);
    if (!ok) {
        for (uint32_t off = 0x60; off <= 0x200; off += 8) {
            void* cand = ReadPtr(chatCls, off);
            if (!cand || !MemReadable(cand, 0x30)) continue;
            if (ObjClass(cand) != chatCls) continue;
    ...
    snprintf(b, sizeof b, "CDO=%p name='%s' offset=0x%x", cdo, cdo ? ObjName(cdo).c_str() : "", g_lay.classDefaultObject);
    add("classDefaultObject", ok, b);

    // 3. FindFunction + UFunction::Func == the .sym address of the exec thunk.
    uint64_t execAddr = Sym::Addr("UPlayerChatComponent::execServer_SendChatMessage");
    void* fn = cdo ? FindFunction(cdo, "Server_SendChatMessage") : nullptr;
    ...
    snprintf(b, sizeof b,
             "FindFunction(CDO,'Server_SendChatMessage')=%p Func=%p sym(execServer_SendChatMessage)=0x%llx offset=0x%x",
             fn, native, (unsigned long long)execAddr, g_lay.funcFunc);
    add("findFunctionNative", ok, b);

    // 4. Property layout (ChildProperties / FField::NamePrivate), self-checked against FindPropertyByName.
    std::string detail;
    void* propCls = StaticClass("ADominionPlayerState::StaticClass");
    if (!propCls) propCls = StaticClass("UItemData::StaticClass");
    if (!propCls) propCls = chatCls;
    ...
    // 5. Property offsets look sane for that class (Offset_Internal < PropertiesSize).
    if (ok) {
    ...
    // 6. GetObjectsOfClass works (uses the object array; world-independent).
    std::vector<void*> objs;
    bool got = GetObjectsOfClass(StaticClass("UObject::StaticClass"), objs, true);
    ...
    bool core = checks.size() >= 3 && checks[0].ok && checks[1].ok && checks[2].ok;
...
void CacheVersionStrings() {
    ...
    auto bv = (FnBuildVersion)(uintptr_t)Sym::Addr("FApp::GetBuildVersion");
    if (bv) {
        const char16_t* s = bv();
        if (MemReadable(s, 2)) {
            int len = 0;
    ...
    auto cur = (FnCurrent)(uintptr_t)Sym::Addr("FEngineVersion::Current");
    auto ts = (FnVerToString)(uintptr_t)Sym::Addr("FEngineVersion::ToString");
    if (cur && ts) {
        const void* v = cur();
...
std::string Reflect::LayoutJson() {
    ...
    return "{" + n("objClass", g_lay.objClass) + n("objName", g_lay.objName) + n("objOuter", g_lay.objOuter) +
           ...
           n("propOffsetInternal", g_lay.propOffsetInternal) + "\"validations\":" + g_validateJson + "}";
}
...
std::string DumpValue(void* obj, void* prop, const std::string& type) {
    ...
    if (type == "StrProperty") {
        ...
        FString s;
        memcpy(&s, p, 16);
        return JsonStr(Reflect::Utf16To8(s.Data, s.Num));
    }
    ...
    for (size_t i = 0; i < chain.size(); i++) {
        ...
        for (void* p : props) {
            ...
            o += "{\"name\":" + JsonStr(NameToString(n)) + ",\"type\":" + JsonStr(type) +
                 ",\"offset\":" + std::to_string(PropertyOffset(p)) + ",\"value\":" + DumpValue(obj, p, type) + "}";
        }
        o += "]}";
    }
    o += "]";
...
std::string Reflect::DumpStruct(const std::string& name) {
    static const char* kPackages[] = {"/Script/Dominion", "/Script/Engine", "/Script/CoreUObject", "/Script/DominionRuntime"};
    void* s = nullptr;
    std::string pkg;
